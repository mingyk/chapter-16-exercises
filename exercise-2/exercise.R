...
ggplot(diamonds) + geom_boxplot(mapping = aes(x = color, y = log(price, base = exp(1))))

# For another version, draw the same plot but with `violin` geometry instead of 
# `boxplot` geometry!
# How does the logarithmic scale change the data presentation?
ggplot(diamonds) + geom_violin(mapping = aes(x = color, y = log(price, base = exp(1))))
...
# for faster results)
ggplot(diamonds_sample, aes(carat, price)) + geom_point(color = clarity)
